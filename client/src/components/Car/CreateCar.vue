<template>
    <div class="container createcafe box">
      ...
      <form v-on:submit.prevent="createCar">
        ...
        <label>Content</label>
        <p>
          <vue-ckeditor v-model.lazy="car.content" :config="config" @blur="onBlur($event)" @focus="onFocus($event)" />
        </p>
        <label>engine</label>
        <input type="text" v-model="car.engine" />
        <label>power</label>
        <input type="text" v-model="car.power" />
        <button type="submit" class="btn btn-success createuser"> CreateCar</button>
        <button v-on:click="navigateTo('/cars')" class="btn btn-warning createuser">Cancel</button>
    ...
    methods: {
      async createCar() {
        try {
          ...
          await CarService.post(this.car);
          this.$router.push({
            name: "cars",
          });
        } catch (err) {
